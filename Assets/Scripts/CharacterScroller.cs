...
{
    [Header("Scroller config")] public float minScale = 1f;
    ...
    [Range(0.1f, 1f)] public float scrollSpeedFactor = 0.25f;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < CharacterManager.Instance.characters.Length; i++)
        {
            ...
            GameObject character = (GameObject) Instantiate(CharacterManager.Instance.characters[i], centerPoint,
                Quaternion.Euler(originalRotation.x, originalRotation.y, originalRotation.z));
            ...
            SpriteRenderer[] charRdr = character.GetComponentsInChildren<SpriteRenderer>();
            ...
            if (charRdr.Length > 0) //This character has childs
            {
                for (int j = 0; j < charRdr.Length; j++)
                {
                    if (charData.IsUnlocked)
                        charRdr[j].color = charData.sawColor;
                    else
                        charRdr[j].color = lockColor;
                }
            }
            else //This character not has child
            {
                if (charData.IsUnlocked)
                    charRdr[0].color = charData.sawColor;
                else
                    charRdr[0].color = lockColor;
            }
    ...
    public void UnlockButton()
    {
        ...
        if (unlockSucceeded)
        {
            SpriteRenderer[] charRdr = currentCharacter.GetComponentsInChildren<SpriteRenderer>();

            if (charRdr.Length > 0)
            {
                for (int i = 0; i < charRdr.Length; i++)
                {
                    charRdr[i].color = currentCharacter.GetComponent<Character>().sawColor;
                }
            }
            else
            {
                currentCharacter.GetComponent<SpriteRenderer>().color =
                    currentCharacter.GetComponent<Character>().sawColor;
            }
